	...
	this.get_authorizations(otp, (err, authorizations) => {
		...
		if (_.has(authorizations, AUTHORIZATION_NAME)) {
			this.remove_authorization(authorizations[AUTHORIZATION_NAME].id, otp, (err) => {
                ...
                github.authorization.create(options, (err, auth) => {
                    console.log('remove create auth err: ' + err);
                    console.log('return remove create auth: ' + JSON.stringify(auth, null, 2));
            ...
            github.authorization.create(options, (err, auth) => {
                console.log('err: ' + err);
                console.log('create auth: ' + JSON.stringify(auth, null, 2));
            });
		}
	});
}
